...
export function resolveCurrentUser(docId: DocumentId): SessionUser {
  ...
  const urlRole = params.get(PERM_KEY) as Permission | null;
  ...
  const permission: Permission =
    urlRole === "admin" || urlRole === "edit" || urlRole === "view"
      ? urlRole
      : (stored.permission ?? "view");
  ...
  // Clean role param from URL so sharing URL stays clean
  if (urlRole) {
...
export function buildShareLinks(docId: DocumentId): {
  ...
  function link(role: Permission): string {
    const u = new URL(base.toString());
    u.searchParams.set(PERM_KEY, role);
    return u.toString();
  }
